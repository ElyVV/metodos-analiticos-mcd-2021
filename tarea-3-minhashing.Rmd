...
```{r}
calcular_tejas(docs[1:2], k = 4) %>% map(unique)
```
...
```
- Se puede calcular todas las similitudes:

```{r}
tejas_tbl <- crossing(id_1 = 1:length(docs), id_2 = 1:length(docs)) %>%
  filter(id_1 < id_2) %>% 
  mutate(tejas_1 = tejas_docs[id_1], tejas_2 = tejas_docs[id_2])
tejas_tbl
```
...
similitud
```
 ---> 164 para el primer documento
 ---> 170 para el segundo documento
...
--> La dimensión de la matriz es de 257 renglones por 3 columnas.

```{r}
dim(dtejas_tbl)
```

--> Tiene 164, que es lo mismo que sale con la pregunta 1.  

```{r}
sum(dtejas_tbl$doc_1 == 1)
```
...
En promedio, el porcentaje de las veces que coincide es un 32%.

```{r}
mean(firmas_perms$doc_1 == firmas_perms$doc_2)
```

Esta cercano este valor en comparación con la similitud de Jaccard.

```{r}
sim_jaccard(tejas_docs[[1]], tejas_docs[[2]])
```

--> Realizo el ejercicio con el cálculo de percentiles.

```{r}
num_perms <- 500
firmas_rep <- map(1:500, function(i){
...
---- Porque para evitar permutar matrices, una manera de hacer esto es trabajar documento por documento, tomamos todos los valores iguales a 1 y aplicamos la permutación (que esta dada por la función perm_fun_1), calculamos el valor mínimo y repetimos. Entonces, al final estamos realizando permutaciones, pero por documento.
...
Nótese que estos nos son números de renglones. ----- ¿Son las funciones?
...
Porque así podríamos evitar permutar matrices (operaciones muy costosas).
...
---- En lugar de usar permutaciones, se usan familias de funciones hash extraidas al azar. En este ejemplo se pueden usar funciones hash modulares (suponiendo que las tejas están enumeradas). 
...
```{r}
crear_tejas_str <- function(docs, k = 4){
    num_docs <- length(docs)
    tejas <- calcular_tejas(docs, k = k)
...
hash_funs <- map(1:500, ~ generar_hash_mult())
